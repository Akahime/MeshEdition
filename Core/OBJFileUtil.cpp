...
void
OBJFileUtil::readToSolid( Solid *mesh, std::istream &in)
{
...
void OBJFileUtil::saveSolid(Solid *mesh, std::ofstream &os)
{
	int vObjID = 1;
...
int
OBJFileUtil:: modifyVertexInf(Solid *sol, char * str)
{
	int vid = 0;
	int tid = 0;
	int nid = 0;
...
*/
void
OBJFileUtil:: modifyNormalInf(Solid * sol, int vid, int nid)
{
	/******************
	std::string temp="";
	if( nid != 0)
...
*/
void
OBJFileUtil::modifyTextureInf(Solid *sol, int vid, int tid)
{
	std::string temp = "";
	if( tid != 0)
	{
...
std::string
OBJFileUtil::d2String( double value)
{
	char buffer [20];
